...
class informe_contabilidad extends fs_controller
{
    ...
    protected function private_core()
    {
        ...
        if (isset($_REQUEST['buscar_subcuenta'])) {
            ...
            //CABMIADO POR GIOVANNI LA FECHA 16/03/2018   $_GET['diario'] X $_POST['diario']
        } else if (isset($_POST['diario'])) {   
            $this->libro_diario_csv($_POST['diario'],$_POST['desde'],$_POST['hasta']);
            ...
            if ($pdf_doc) {
                ...
                for ($i = $linea; $i < min(array($linea + 48, count($lineas))); $i++) {
                    if (strlen($lineas[$i]['cuenta']) == 1) {
                        ...
                        $tdeudor += $lineas[$i]['deudor'];
                        $tacreedor += $lineas[$i]['acreedor'];
                    ...
                    $pdf_doc->add_table_row(
                        array(
                            ...
                            'deudor' => $a . $this->show_numero($lineas[$i]['deudor']) . $b,
                            'acreedor' => $a . $this->show_numero(floatval($lineas[$i]['acreedor']) ) . $b
                        )
                    );
                }
                $linea += 48;
                ...
                $pdf_doc->add_table_row(
                    array(
                        ...
                        'deudor' => '<b>' . $this->show_numero($tdeudor) . '</b>',
                        'acreedor' => '<b>' . $this->show_numero($tacreedor) . '</b>'
                    )
                );
                $pdf_doc->save_table(
                    array(
                        ...
                        'cols' => array(
                            ...
                            'deudor' => array('justification' => 'right'),
                            'acreedor' => array('justification' => 'right')
                        ),
                        'width' => 540,
                        'shaded' => 0
                    )
                ...
                for ($i = $linea; $i < min(array($linea + 48, count($lineas))); $i++) {
                    ...
                    number_format($lineas[$i]['deudor'], FS_NF0, ',', '') . ';' .
                    number_format(floatval($lineas[$i]['acreedor']), FS_NF0, ',', '') . "\n";
                }
                $linea += 48;
            }
        }
